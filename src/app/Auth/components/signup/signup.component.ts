...
export class SignupComponent implements OnInit {
  ...
  ngOnInit() {
    this.signupFrom = this.fb.group({
      ...
      emailGroup: this.fb.group({
        email: ['', [Validators.required, Validators.email, CustomValidators.emailCustomValidation("yahoo.com")]],
    ...
    this.signupFrom.get('contactPreference')
  ...
  confirmEmailMatchValidator(group: AbstractControl): { [key: string]: any } | null {
    const emailControl = group.get('email').value;
    const confirmEmailControl = group.get('confirmEmail').value;
    if (emailControl == confirmEmailControl || (group.get('confirmEmail').pristine && confirmEmailControl == '')) {
      return null;
    }
    else {
  ...
  signup() {
    ...
    this._authService.signup(this.user).subscribe((data: any) => {
